...
def get_bookinfo(bookId):
    ...
    data = request_data(url)
    bookinfo = [('title', data['title']), ('author', data['author']), ('category', data['category']),
                ('introduction', data['intro']), ('publisher', data['publisher'])]
    res = '\n'
...
def get_comment(bookId, mode='1'):
    url = 'https://i.weread.qq.com/review/list?listType=6&synckey=0&userVid=' + str(
        USERVID) + '&rangeType=2&mine=1&listMode=1'
    data = request_data(url)
...
def get_new_content_byrange(bookId):
    ...
    if os.path.isfile(temp_dir + "\\range.txt"):
        ...
        range_and_text = (
        sorted_contents[last_chapterUid][-1][0], last_chapterUid, sorted_contents[last_chapterUid][-1][2])
        # 更新range.txt
...
if __name__ == '__main__':
    ...
    # for b in books:
    #     print(b)
    # print(get_bookinfo(25469762))
    # for c in get_chapters(25469762):
    #     print('#' * c[0], c[1])
